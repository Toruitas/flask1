...
class APITestCase(unittest.TestCase):
    ...
    def test_no_auth(self):
        """
        Posts should still be viewable without authorization
        :return:
        """
    ...
    def test_posts(self):
        """
        adds user to db then uses restful api to insert a blog post, then read it back. Any requests that send data in
        the body must encode it with json.dumps(), since Flask test client doesn't automatically encode to JSON.
        Likewise, response bodies are also returned in JSON format and must be decoded with json.loads() before
        can be inspected.
        :return:
        """
        # add a user
